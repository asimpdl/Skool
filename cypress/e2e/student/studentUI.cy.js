...
describe('Programs UI and Search Button Checks', () => {
  beforeEach(() => {
    cy.visit('https://app.acharyatech.com')
    // Navigate to program
    cy.get('a[href="/student"]').click()
  ...
  it('Programs Search Functionality', () => {
    cy.get('.MuiCardHeader-root').should('exist').and('be.visible')
      .within(() => {
        cy.get('form > .MuiBox-root').should('exist').and('be.visible')
        cy.get('.MuiInputBase-input').should('exist').and('be.visible').type('BC')
        cy.get('.MuiButtonBase-root').should('exist').and('be.visible').click()
      });
    
    cy.get('.MuiCardContent-root > .MuiBox-root').should('exist').and('be.visible')
      .within(() => {
    cy.get('svg[role="img"]').should('exist').and('be.visible')
    cy.get('p').should('contain.text', 'BC').and('be.visible')
      }); 
  });
});
...
describe('Admission UI and its Functionality', () => {
  ...
  it('Admision Data Visible', () => {
    //MuiDataGrid-topContainer
    cy.get('.MuiCardContent-root').should('exist').and('be.visible')
      .within(() => {
      cy.get('.MuiDataGrid-topContainer').should('exist').and('be.visible')
      cy.get('.MuiDataGrid-virtualScrollerContent').should('exist').and('be.visible')
    })
  });
 });